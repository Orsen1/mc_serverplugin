...
public class PlayerData {
    ...
    int total = 0;
    int covered = 0; // колонок, где небо НЕ видно
    ...
    double ratio = covered / (double) total; // доля закрытых колонок

    // Порог: например, считаем “нет неба”, если закрыто >= 80% колонок
    return ratio >= 0.8;
}
...
private static boolean isSkyBlockedInColumn(World world, int x, int startY, int z) {
    ...
    private static double wallFactor(World world, Location loc, int radius) {
        ...
        for (int dx = -radius; dx <= radius; dx++) {
            for (int dz = -radius; dz <= radius; dz++) {
                if (dx == 0 && dz == 0) continue; // центр не считаем
                total++;
